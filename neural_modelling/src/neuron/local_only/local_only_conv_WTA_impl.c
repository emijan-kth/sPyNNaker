...
bool local_only_impl_initialise(void *address){
    ...
    if (local_only_conv_config == NULL) {
    	log_error("Can't allocate memory for config!");
    	return false;
    }
    spin1_memcpy(local_only_conv_config, sdram_config, n_bytes);

    log_info("post_start = %u, %u, post_end = %u, %u, post_shape = %u, %u",
            local_only_conv_config->post_start.col, local_only_conv_config->post_start.row,
            local_only_conv_config->post_end.col, local_only_conv_config->post_end.row,
            local_only_conv_config->post_shape.width, local_only_conv_config->post_shape.height);
    log_info("num sources = %u", local_only_conv_config->n_sources);

    if (local_only_conv_config->n_sources == 0) {
    ...
    connector *sdram_connectors =
    		(connector *) &(sdram_config->sources[local_only_conv_config->n_sources]);
    uint32_t n_connector_bytes = sizeof(connector) * local_only_conv_config->n_connectors_total;
    connectors = spin1_malloc(n_connector_bytes);
    if (connectors == NULL) {
    	log_error("Can't allocate %u bytes of memory for %u connectors!",
    			n_connector_bytes, local_only_conv_config->n_connectors_total);
    	return false;
    }
    ...
    lc_weight_t *kernel_weights =
    		(lc_weight_t *) &(sdram_connectors[local_only_conv_config->n_connectors_total]);
    uint32_t n_weight_bytes = sizeof(lc_weight_t) * local_only_conv_config->n_weights_total;
    weights = spin1_malloc(n_weight_bytes);
    if (weights == NULL) {
    	log_error("Can't allocate %u bytes of memory for %u weights!",
    			n_weight_bytes, local_only_conv_config->n_weights_total);
    	return false;
    }
    ...
    for (uint32_t i = 0; i < local_only_conv_config->n_sources; i++) {
        ...
        log_debug("    is_WTA_reset=%s", s_info->is_WTA_reset ? "true" : "false");
    }

    for (uint32_t i = 0; i < local_only_conv_config->n_connectors_total; i++) {
...
static inline void do_convolution_operation(
    ...
    for (int32_t i_row = start_i.row, tmp_row = post_coord.row; i_row < connector->kernel.height; i_row += connector->strides.row, --tmp_row) {
        ...
        if ((tmp_row < local_only_conv_config->post_start.row) || (tmp_row > local_only_conv_config->post_end.row)) {
        ...
        for (int32_t i_col = start_i.col, tmp_col = post_coord.col; i_col < connector->kernel.width; i_col += connector->strides.col, --tmp_col) {
            ...
            if ((tmp_col < local_only_conv_config->post_start.col) || (tmp_col > local_only_conv_config->post_end.col)) {
            ...
            uint32_t post_index =
                ((tmp_row - local_only_conv_config->post_start.row) * local_only_conv_config->post_shape.width)
                    + (tmp_col - local_only_conv_config->post_start.col);
