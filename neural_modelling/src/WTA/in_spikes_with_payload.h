...
#define BUFFER_ITEM_TYPE spike_t
#include "spike_circular_buffer.h"
...
static spike_circular_buffer buffer;
...
static inline bool in_spikes_initialize_spike_buffer(uint32_t size) {
    buffer = spike_circular_buffer_initialize(size);
    return buffer != 0;
}
...
static inline bool in_spikes_add_spike(spike_t spike) {
    return spike_circular_buffer_add(buffer, spike);
}
...
static inline bool in_spikes_get_next_spike(spike_t* spike) {
    return spike_circular_buffer_get_next(buffer, spike);
}
...
static inline bool in_spikes_is_next_spike_equal(spike_t spike) {
    return spike_circular_buffer_advance_if_next_equals(buffer, spike);
}
...
static inline counter_t in_spikes_get_n_buffer_overflows(void) {
    return spike_circular_buffer_get_n_buffer_overflows(buffer);
}
...
static inline void in_spikes_print_buffer(void) {
    spike_circular_buffer_print_buffer(buffer);
}
...
static inline uint32_t in_spikes_input_index(void) {
    return spike_circular_buffer_input(buffer);
}
...
static inline uint32_t in_spikes_output_index(void) {
    return spike_circular_buffer_output(buffer);
}
...
static inline spike_t in_spikes_value_at_index(uint32_t index) {
    return spike_circular_buffer_value_at_index(buffer, index);
}
#endif // _IN_SPIKES_H_
