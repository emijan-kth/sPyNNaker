...
static void timer_callback(UNUSED uint unused0, UNUSED uint unused1) {
    ...
    {
        neuron_id_in_has_spike[i] = false;
    }
...
bool local_only_initialise(void *address) {
    ...
    neuron_id_in_has_spike = spin1_malloc(num_neurons_in * sizeof(neuron_id_in_has_spike[0]));

    if (neuron_id_in_has_spike == NULL) {
